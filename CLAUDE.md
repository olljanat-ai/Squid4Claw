...
- **Default-deny**: All connections blocked until admin approves (globally or per-skill)
...
internal/
  ...
  approval/          - Default-deny approval system (pending/approved/denied)
...
- **Global vs per-skill approvals**: Approvals with empty `skill_id` are global (all agents). Skill-specific approvals only apply to that skill's token. Global approvals are checked first.
- **Anonymous access**: Agents can make requests without skill tokens. These go through the global approval system. Invalid tokens are rejected; missing tokens are anonymous.
...
- **Approval flow**: `Check()` registers a pending entry, `WaitForDecision()` blocks until admin decides or timeout, `Decide()` notifies all waiters via channels.
...
- The `checkApproval()` function checks: pre-approved hosts -> global approvals -> skill-specific approvals -> register pending and wait
